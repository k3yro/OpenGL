...
void main()
{
	// Vektor von Fragment zu Kamera (Kamera immer an Position 0,0,0)
	...
	vec3 specular = u_directional_light.specular * pow(max(dot(reflection, view), 0.0000001), 1/*u_material.shininess*/) * u_material.specular;

	// Punktlicht (z.B. Stassenlaterne, Scheinwerfer, ...)
	...
	ambient += attentuation * u_point_light.specular * pow(max(dot(reflection, view), 0.0000001), 1/*u_material.shininess*/) * u_material.specular;
